...
mod_volley_covariates <- "
  ...
  for (g in 1:nlength_set){
      ...
      u[g] ~ dbern(po[g])
      logit(po[g]) <- iota +
                           alpha*(c.1.1*(abs(p.ab[team1[g]]-p.ab[team2[g]]))+
                                   c.2.1*(p.ab[team1[g]]-p.ab[team2[g]])^2 )     
                           + beta*( c.1.2*(abs(set.ab[team1[g]]-set.ab[team2[g]]))+
                                    c.2.2*(set.ab[team1[g]]-set.ab[team2[g]])^2)
  ...
  for ( t in 1:nteams){
    ...
    alpha ~ dnorm(0, 0.001)     # delta
    beta ~ dnorm(0, 0.001)      # gamma
