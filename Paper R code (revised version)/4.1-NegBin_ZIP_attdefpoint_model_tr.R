...
mod_volley_att <- "
  ...
  for (g in 1:nlength_set){
      ...
      logit(pp[g]) <- mu+(1-Z[g])*p.home+(p.ab.att[team1[g]]-p.ab.def[team2[g]])*(1-Z[g])+
                      (p.ab.att[team2[g]]-p.ab.def[team1[g]])*Z[g]
  ...
  for ( t in 1:nteams){
      p.ab.att[t] <- p.ab.att.star[t]-mean(p.ab.att.star[])
      p.ab.def[t] <- p.ab.def.star[t]-mean(p.ab.def.star[])
      set.ab[t] <- set.ab.star[t]-mean(set.ab.star[])
      p.ab.att.star[t]  ~ dnorm(0, 0.5)
      p.ab.def.star[t]  ~ dnorm(0, 0.5)
...
# fitting
volley.jags <- run.jags(model=mod_volley_att, 
                        data=data.jags.ita, 
                        monitor=moni, 
