...
mod_volley <- "
  ...
  for ( t in 1:nteams){
      ...
      p.ab.star[t]  ~ dnorm(0, 1)
      set.ab.star[t]~ dnorm(0, 1)
 }
...
# fitting
volley.jags <- run.jags(model=mod_volley, 
                        data=data.jags.ita, 
                        monitor=moni, 
